...
- **Offen (nächste Sprints lt. Plan):** P1 Strict-Lock-Zeitautorität (nativ `StrictLockClockGuard` existiert; prüfen ob JS-Schicht ihn via `Date.now()` unterläuft), Penalty-Idempotenz über Mount/Crash + Preimage-Gating, Defense-in-Depth-Guards (Triage (b)); P1/P2 Performance-Hot-Path (`submitReview` O(n), Persist-Debounce, dreifacher Queue-Build); P2 Lern-Qualität (Typed-„partial"→hard, Transliteration, Optimizer-Ehrlichkeit, Leech-Sichtbarkeit).
