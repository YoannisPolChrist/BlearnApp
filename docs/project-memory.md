...
## 23. User-Reports „Apps nicht freischaltbar" + „Vokabeln doppelt" (2026-06-14)

- **Unlock-Zuverlässigkeit:** Untersucht. Bestätigte Ursache von „Learn-Gate-App schaltet nicht frei" = die **Blocked-Flow-Sackgasse**, bereits in P0 (Abschnitt 22) behoben — aber noch nicht aufs Gerät deployed. Eine spekulative Normalisierung in `primeNativeUnlockHandoff` (trim→lowercase) wurde **verworfen**: die Tests (`androidOverlaySuccess`) verankern bewusst Pass-Through der Original-ID an native (`grantManualOverride`/`openTarget` brauchen die echte Kennung zum Re-Launch; native reconciliert Groß/Klein selbst — Store/Snapshot füttern bereits lowercase und es funktioniert). Kein zusätzlicher verifizierbarer JS-Bug; alles Weitere braucht Geräte-Repro (welche App, welcher Modus, was genau).
- **Vokabeln doppelt:** Innerhalb EINER Session ist `buildUnlockSessionCandidateIds` per Konstruktion eindeutig — AUSSER bei `burySiblings=false` (dann greift die Geschwister-Dedup nicht). Als Absicherung explizite **ID-Eindeutigkeits-Invariante** am Ende von `buildUnlockSessionCandidateIds` ergänzt (keine Karte erscheint zweimal, unabhängig von Preset/Quellen). Test in `learning.test.ts`. Cross-Flow-Wiederkehr derselben Vokabel war der FSRS-Fuzz-Bug (Abschnitt 20, behoben) bzw. korrektes SRS-Verhalten bei Lernkarten/again — ebenfalls noch nicht aufs Gerät deployed.
- **Wichtig:** Branch `fix/masterplan-phase0-bugfixes` enthält die Fixes; **Geräte-Build/Deploy steht aus**. Viele Reports sind vermutlich gegen einen älteren Build.

## 24. Post-Flow-Regressionen: „lädt ewig" + „keine Emotionen mehr" (2026-06-15)

User-Report seit dem Success-Screen-Change: (1) der Freischalt-Button/-Screen „lädt ewig" nach einem geschafften Flow, (2) im Blocking-Flow wird kein Emotions-Check-in mehr abgefragt (User will ihn zurück).

- **(1) Lade-Hänger:** `finishUnlock` (`useLearnReviewSessionCompletion`) wartete VOR dem Erfolgs-Screen per `await` auf `waitForBlockingFlowPersistence` (bis ~5s) + `withTimeout(flushLearningCloudSaveIfAvailable, 3500)`. Bei langsamem/abgehängtem Sync drehte der Screen mehrere Sekunden. Fix: Freischaltung synchron (`runPendingReviewWrites` + `registerUnlockGrant` + `unlockTarget`), Erfolgs-Screen SOFORT (`setOverlaySuccessVisible(true)`); Persistenz + Cloud-Save laufen jetzt fire-and-forget im Hintergrund. Durability bleibt (WAL + unlockTarget sind synchron). Der CTA `handleContinueToTarget` wartete ohnehin nie auf Persistenz.
- **(2) Emotionen zurück:** `handleReview` setzte im Unlock-Abschluss `awaitingEmotionSelection(false)` + `blockedUnlockSignal` (Emotions-Skip). Jetzt zeigen BEIDE Flows den Emotions-Check-in (`setAwaitingEmotionSelection(true)`); `completeSessionEmotionStep` ruft danach `finishUnlock`. **Sicher gegen den alten #10-Bug**, weil finishUnlock die Freischaltung synchron erteilt + den Erfolgs-Screen ohne await zeigt → kann nie an einem hängenden Async-Schritt scheitern. Emotionen sind PFLICHT (≥1), wie im Learn-Hub-Flow (User-Wunsch). Der `blockedUnlockSignal`-Effekt ist jetzt dormanter Legacy-Fallback (nie getriggert).
- **Agenten-Worktree-Falle (erneut):** Der beauftragte Subagent landete wieder auf dem Skeleton-Commit `2b32c09` (ohne meine P0/P1/P2-Arbeit) statt auf `fix/masterplan-phase0-bugfixes`. Sein Commit war daher nicht cherry-pick-bar (hätte meine Arbeit überschrieben) — Design übernommen, auf echtem HEAD reimplementiert, Worktree verworfen. **Lehre:** Agenten-Worktrees können auf einem Stale-Base starten; immer die Base prüfen und ggf. neu auf den Ziel-Branch reimplementieren statt cherry-picken.
- **Verifikation:** `tsc` clean; `androidOverlaySuccess` (16), `learnReviewUi` (17), `appBlockingFallback`/`checkinPage`/`sessionController` (26) grün — inkl. neuer Test „shows the learn success screen … even while background persistence stalls".
