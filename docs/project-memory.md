...
- **Offen (nächste Sprints lt. Plan):** P1-A Strict-Lock-Zeitautorität (nativ `StrictLockClockGuard` existiert; prüfen ob JS-Schicht ihn via `Date.now()` unterläuft, L+Gerät); P1/P2 Performance-Hot-Path (`submitReview` O(n), Persist-Debounce, dreifacher Queue-Build); P2 Lern-Qualität (Typed-„partial"→hard, Transliteration, Optimizer-Ehrlichkeit, Leech-Sichtbarkeit).
