...
import { countNewCardsIntroducedToday, countReviewsToday, getWeeklyReviewActivity } from './utils';
...
export function getDeckLearningStats(options: {
  ...
  now?: number;
  /** Store callers can avoid a second filter/migration after scoping cards. */
  cardsAreScopedAndNormalized?: boolean;
}): LearningDeckStats {
  const now = options.now ?? Date.now();
  const resolvedPreset = migrateLearningPreset(options.preset);
  const resolvedGateRule = migrateGateRule(options.gateRule);
  const deckCards = options.cardsAreScopedAndNormalized
    ? options.cards
    : options.cards
  ...
  const weeklyActivity = getWeeklyReviewActivity(options.reviewLogs, options.deck.id, now);
  ...
  return {
    ...
    neverLearnedCount: deckCards.filter((card) => card.reps === 0 && !card.lastReviewedAt).length,
    reviewsThisWeek: weeklyActivity.reviewsThisWeek,
    activeDaysThisWeek: weeklyActivity.reviewedDaysThisWeek.filter(Boolean).length,
    reviewedDaysThisWeek: weeklyActivity.reviewedDaysThisWeek,
