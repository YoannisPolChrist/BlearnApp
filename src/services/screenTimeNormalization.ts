  ...
  return {
    ...
    accessibilityServiceDisconnectedAt:
      typeof status?.accessibilityServiceDisconnectedAt === 'number' && Number.isFinite(status.accessibilityServiceDisconnectedAt)
        ...
        : undefined,
    usageStatsPermission: Boolean(status?.usageStatsPermission),
    deviceAdminActive: Boolean(status?.deviceAdminActive),
    batteryOptimizationExempt: Boolean(status?.batteryOptimizationExempt),
    notificationsEnabled: Boolean(status?.notificationsEnabled),
    privateDnsMode: normalizeText(status?.privateDnsMode) || undefined,
    vpnInterruptedByBoot: Boolean(status?.vpnInterruptedByBoot),
