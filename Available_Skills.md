...
- **00-andruia-consultant**: "Arquitecto de Soluciones Principal y Consultor TecnolÃ³gico de Andru.ia. Diagnostica y traza la hoja de ruta Ã³ptima para proyectos de IA en espaÃ±ol."
- **10-andruia-skill-smith**: "Ingeniero de Sistemas de Andru.ia. DiseÃ±a, redacta y despliega nuevas habilidades (skills) dentro del repositorio siguiendo el EstÃ¡ndar de Diamante."
- **20-andruia-niche-intelligence**: "Estratega de Inteligencia de Dominio de Andru.ia. Analiza el nicho especÃ­fico de un proyecto para inyectar conocimientos, regulaciones y estÃ¡ndares Ãºnicos del sector. ActÃ­valo tras definir el nicho."
...
- **anti-reversing-techniques**: "Understand anti-reversing, obfuscation, and protection techniques encountered during software analysis. Use when analyzing protected binaries, bypassing anti-debugging for authorized analysis, or u..."
...
- **automate-whatsapp**: "Build WhatsApp automations with Kapso workflows: configure WhatsApp triggers, edit workflow graphs, manage executions, deploy functions, and use databases/integrations for state. Use when automatin..."
...
- **blockrun**: "Use when user needs capabilities Claude lacks (image generation, real-time X/Twitter data) or explicitly requests external models (\\\"blockrun\\\", \\\"use grok\\\", \\\"use gpt\\\", \\\"da..."
...
- **cc-skill-security-review**: "Use this skill when adding authentication, handling user input, working with secrets, creating API endpoints, or implementing payment/sensitive features. Provides comprehensive security checklist a..."
...
- **commit**: "Create commit messages following Sentry conventions. Use when committing code changes, writing commit messages, or formatting git history. Follows conventional commits with Sentry-specific issue re..."
...
- **comprehensive-review-pr-enhance**: "You are a PR optimization expert specializing in creating high-quality pull requests that facilitate efficient code reviews. Generate comprehensive PR descriptions, automate review processes, and e..."
...
- **crypto-bd-agent**: Autonomous crypto business development patterns â€” multi-chain token discovery, 100-point scoring with wallet forensics, x402 micropayments, ERC-8004 on-chain identity, LLM cascade routing, and...
...
- **d3**: "Build custom, bespoke data visualizations with D3.js (Data-Driven Documents). The low-level, SVG-based approach to any chart imaginable. Use when Tremor/ECharts lack control, or when building animated, custom, or mathematical visualizations."
...
- **data-structure-protocol**: "Give agents persistent structural memory of a codebase â€” navigate dependencies, track public APIs, and understand why connections exist without re-reading the whole repo."
...
- **drizzle-orm-expert**: "Expert in Drizzle ORM for TypeScript â€” schema design, relational queries, migrations, and serverless database integration. Use when building type-safe database layers with Drizzle."
...
- **echarts**: "Build powerful, interactive charts using Apache ECharts. Use when you need complex charts (bar, line, pie, scatter, heatmap, tree, radar, gauge, candlestick), rich interactivity, large datasets, or mobile-first charts."
...
- **form-cro**: Optimize any form that is NOT signup or account registration â€” including lead capture, contact, demo request, application, survey, quote, and checkout forms.
...
- **gemini-3-pro-coder**: Specialist for coding with Gemini 3.1 Pro. Use this skill when the user explicitly requests Gemini 3 models or high-end reasoning.
...
- **google-calendar-automation**: "Automate Google Calendar events, scheduling, availability checks, and attendee management via Rube MCP (Composio). Create events, find free slots, manage attendees, and list calendars programmatica..."
- **google-drive-automation**: "Automate Google Drive file operations (upload, download, search, share, organize) via Rube MCP (Composio). Upload/download files, manage folders, share with permissions, and search across drives pr..."
...
- **internal-comms-anthropic**: "A set of resources to help me write all kinds of internal communications, using the formats that my company likes to use. Claude should use this skill whenever asked to write some sort of internal ..."
- **internal-comms-community**: "A set of resources to help me write all kinds of internal communications, using the formats that my company likes to use. Claude should use this skill whenever asked to write some sort of internal ..."
...
- **lightning-factory-explainer**: Explain Bitcoin Lightning channel factories and the SuperScalar protocol â€” scalable Lightning onboarding using shared UTXOs, Decker-Wattenhofer trees, timeout-signature trees, MuSig2, and Taproot. No soft fork required.
...
- **motion-react-animations**: "Expert patterns for the motion library (motiondivision/motion, formerly Framer Motion). Use when adding premium web animations to React or React Native (Expo) apps â€” enter/exit, gestures, scroll, layout, and shared element transitions. On React Native use in combination with moti/reanimated for cross-platform."
...
- **nerdzao-elite-gemini-high**: "Modo Elite Coder + UX Pixel-Perfect otimizado especificamente para Gemini 3.1 Pro High. Workflow completo com foco em qualidade mÃ¡xima e eficiÃªncia de tokens."
...
- **observe-whatsapp**: "Observe and troubleshoot WhatsApp in Kapso: debug message delivery, inspect webhook deliveries/retries, triage API errors, and run health checks. Use when investigating production issues, message f..."
...
- **odoo-migration-helper**: "Step-by-step guide for migrating Odoo custom modules between versions (v14â†’v15â†’v16â†’v17). Covers API changes, deprecated methods, and view migration."
...
- **odoo-purchase-workflow**: "Expert guide for Odoo Purchase: RFQ â†’ PO â†’ Receipt â†’ Vendor Bill workflow, purchase agreements, vendor price lists, and 3-way matching."
...
- **project-patterns-therapyclient**: "Project specific patterns and technical designs for the TherapyClientApp"
...
- **research-engineer**: "An uncompromising Academic Research Engineer. Operates with absolute scientific rigor, objective criticism, and zero flair. Focuses on theoretical correctness, formal verification, and optimal impl..."
...
- **terraform-aws-modules**: "Terraform module creation for AWS â€” reusable modules, state management, and HCL best practices. Use when building or reviewing Terraform AWS infrastructure."
...
- **tremor**: "Build beautiful React chart components and dashboards with Tremor. Copy-paste components built on Recharts and Tailwind CSS. Use for BarChart, LineChart, AreaChart, DonutChart, ScatterChart, and KPI cards."
...
- **vercel-deploy-claimable**: "Deploy applications and websites to Vercel. Use this skill when the user requests deployment actions such as 'Deploy my app', 'Deploy this to production', 'Create a preview deployment', 'Deploy and..."
...
- **vizzu-lib**: "Build animated data visualizations and interactive data stories using Vizzu. Use when creating charts, graphs, and animating transitions between different data perspectives."
...
- **x-twitter-scraper**: "X (Twitter) data platform skill â€” tweet search, user lookup, follower extraction, engagement metrics, giveaway draws, monitoring, webhooks, 19 extraction tools, MCP server."
